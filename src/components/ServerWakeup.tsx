...
export function ServerWakeup({ onReady }: Props) {
  ...
  useEffect(() => {
    ...
    Animated.timing(logoFade, {
      toValue: 1, duration: 1100, delay: 200, useNativeDriver: true,
    }).start();
  }, []);
  ...
  const msg = {
    checking: 'Préparation de votre cave…',
    waking:   'Organisation de votre cave en cours…',
    ready:    'Cave prête !',
      ...
      <Animated.View style={[s.content, { transform: [{ scale: scaleAnim }] }]}>
        ...
        <Animated.View style={{ opacity: logoFade }}>
          <View style={s.brandBlock}>
            <Text style={s.wordmark}>CAVOU</Text>
            <View style={s.wordSep} />
            <Text style={s.brandTagline}>Votre cave personnelle</Text>
          </View>
        </Animated.View>
...
const s = StyleSheet.create({
  ...
  wordSep: {
    width: 32, height: 1,
    backgroundColor: Colors.ambreChaud + '70',
  },
  brandTagline: {
  ...
  separator: {
    width: 44, height: 1,
    backgroundColor: Colors.ambreChaud + '35',
